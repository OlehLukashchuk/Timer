new Vue({
  ...
  data: {
    ...
    time_text: "",
    percent_done: 100,
    endingAudio: document.getElementById(`${localStorage.getItem("Audio")}`),
  },
  ...
  methods: {
    ...
    set_interval: function () {
      this.interval = setInterval(() => {
        this.time_text = moment
          .utc(this.end_ts - Date.now())
          .format("HH:mm:ss");

        this.percent_done =
          (100 * (this.end_ts - Date.now())) / (this.session_time * 60 * 1000);
    ...
    button_click: function (name, detail, event) {
      if (name === "start_again") {
        ...
        let session_duration_in_mili_seconds = this.session_time * 60 * 1000;
        let five_seconds_in_mili_seconds = 5 * 1000;
        console.log(session_duration_in_mili_seconds);
        this.end_ts = this.start_ts + five_seconds_in_mili_seconds;
        // this.end_ts = this.start_ts + this.session_time * 60 * 1000;
        // this.end_ts = this.start_ts + this.session_time * 60 * 10;
        console.log(this.start_ts, this.end_ts, this.end_ts - this.start_ts);
        ...
        this.active_page_name = "select_minutes";
      } else if (name === "audioChoose") {
        let chosenAudio = event.target.previousElementSibling.id;

        if (localStorage.getItem("Audio")) {
          localStorage.removeItem("Audio");
          localStorage.setItem("Audio", chosenAudio);
        } else {
          localStorage.setItem("Audio", chosenAudio);
        }

        let audio_from_localStorage = document.getElementById(
          `${localStorage.getItem("Audio")}`
        );
        this.endingAudio = audio_from_localStorage;
    ...
    drawer_item_click: function (item_name) {
      ...
      if (item_name === "license") {
        ...
        this.active_page_name = "chooseAudio";
